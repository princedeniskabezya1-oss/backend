...
router.post("/request", auth, async (req,res) => {
  try{
    if(req.user.role !== "family"){
      return res.status(403).json({ message:"Family account required" });
    }
    ...
    if(!familyChildId || !aiftStudentId || !allowedRelationships.has(relationshipType)){
      return res.status(400).json({ message:"Child, AIFT Student ID and relationship are required" });
    }

    const child = await FamilyChild.findOne({
      _id:familyChildId,
      familyId:userId(req.user),
      status:{ $ne:"archived" }
    });
    ...
    const student = await User.findOne({ _id:identity.studentId, role:"student", status:"active" })
      .select("_id name");
    if(!student) return res.status(404).json({ message:"Verified student account not found" });

    const existing = await FamilyStudentLinkRequest.findOne({
      familyId:userId(req.user), familyChildId:child._id, studentId:student._id, status:"pending"
    });
    if(existing) return res.status(409).json({ message:"A link request is already pending" });

    const request = await FamilyStudentLinkRequest.create({
      familyId:userId(req.user),
      familyChildId:child._id,
    ...
    await Notification.create({
      ...
      text:`A Family account requested to connect as your ${relationshipType.replaceAll("_"," ")}.`,
...
router.get("/student/pending", auth, async (req,res) => {
  try{
    ...
    await FamilyStudentLinkRequest.updateMany(
      { studentId:userId(req.user), status:"pending", expiresAt:{ $lte:new Date() } },
      { $set:{ status:"expired" } }
    );
...
router.patch("/:id/respond", auth, async (req,res) => {
  try{
    ...
    const request = await FamilyStudentLinkRequest.findOne({
      _id:req.params.id,
      studentId:userId(req.user),
      status:"pending"
    });
    ...
    if(child){
      if(decision === "accept"){
        const conflict = await FamilyChild.exists({
          familyId:request.familyId,
          linkedStudentId:userId(req.user),
          _id:{ $ne:child._id },
          status:{ $ne:"archived" }
        });
        if(conflict) return res.status(409).json({ message:"This student is already linked in that Family account" });
        child.linkedStudentId = userId(req.user);
        child.linkStatus = "linked";
      }else{
        child.linkedStudentId = null;
        child.linkStatus = "unlinked";
      }
      await child.save();
    }
...
router.patch("/:id/revoke", auth, async (req,res) => {
  try{
    ...
    await FamilyChild.updateOne(
      { _id:request.familyChildId, familyId:request.familyId, linkedStudentId:userId(req.user) },
      { $set:{ linkedStudentId:null, linkStatus:"unlinked" } }
    );
    ...
    await Notification.create({
      ...
      sender:userId(req.user),
      text:"An AIFT student revoked a Family account connection.",
      link:"/family.html"
    });
