...
router.post("/", auth, upload.single("cv"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      finalCvUrl = uploadResult.secure_url;
      cvSource = "uploaded";
    }
...
========================================= */
    ...
    if (io) {
      io.to(String(job.employerId))
        .emit("application_created", application);
    }
  ...
  try {
    ...
    if (actor.role === "admin") {
      ...
      const apps = await Application.find()
        ...
        .populate(
          "applicantId",
          "name email profileImage headline role cvUrl skills"
        )
        .sort({ createdAt: -1 });
    ...
    if (actor.role === "employer") {
      ...
      const apps = await Application.find({
        ...
        .populate(
          "applicantId",
          "name email profileImage headline role cvUrl skills education experience expectedSalary"
        )
        .sort({ createdAt: -1 });
...
router.patch("/:id/status", auth, async (req, res) => {
  try {
    ...
    await Notification.create({
      ...
      sender: actor._id,
      text:
        `Your application for ${application.jobId.title} is now ${req.body.status}`,
    ...
    if (io) {
      io.to(String(application.applicantId))
        .emit("application_status_updated", {
...
router.patch("/:id/follow-up", auth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "You can only follow up on your own application"
      });
    }
    ...
    if (hoursPassed < 24) {
      return res.status(400).json({
        message:
          "You can follow up after 24 hours"
      });
    }
    ...
    await Notification.create({
      ...
      sender: userId,
      text:
        `${sender?.name || "Applicant"} followed up on their application for ${application.jobId?.title || "your job"}`,
      link: "/employer.html?tab=pipeline"
    });
    ...
    if (io) {
      io.to(String(application.employerId))
        .emit("application_follow_up", {
          applicationId: application._id,
          message
        });
    }
