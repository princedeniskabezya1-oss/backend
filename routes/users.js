...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    const impressionRows = await Post.aggregate([
      {
        $match: {
          author: user._id,
      ...
      },
      {
        $group: {
          _id: null,
          total: { $sum: { $ifNull: ["$viewsCount", 0] } }
        }
      }
    ]);

    const postImpressions = Number(impressionRows?.[0]?.total || 0);
...
router.patch("/:id/profile-view", auth, async (req, res) => {
  try {
    ...
    const target = await User.findById(targetId)
      .select("_id name companyName schoolName role profileViews uniqueProfileViewers isPublic status");
    ...
    const viewer = await User.findById(viewerId)
      .select("_id name companyName schoolName profileImage role status")
      .lean();

    if (!viewer || viewer.status === "suspended") {
